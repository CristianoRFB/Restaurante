...
- O modo demo é local e não representa autenticação de produção.
...
Riscos restantes: conectar Firebase Auth real, implementar rate limiting/antiabuso para reservas públicas, ativar App Check no domínio final, revisar headers/CSP do Worker e fazer teste de dependências em pipeline CI.
