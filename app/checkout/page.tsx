...
import { clearTableSession, createClientRequestId, createOrderAsync, defaultOrderSettings, readOrderCatalogAsync, readOrderSettingsAsync, readTableSession, writeTableSession } from '@/lib/order-repository';
import { readSettingsAsync } from '@/lib/baru-repository';
import { calculateCartPreview, formatOrderMoney, sanitizeTableId, type FulfillmentMode, type OrderCatalog, type OrderOperationsSettings, type PaymentMethod } from '@/shared/order-domain';
...
export default function CheckoutPage() {
  ...
  const [fields, setFields] = useState({ name: '', whatsapp: '', email: '', street: '', number: '', complement: '', neighborhood: '', reference: '', notes: '', changeFor: '', promoCode: '', zoneId: '', tableId: readTableSession() });
  const [tableLocked, setTableLocked] = useState(() => Boolean(readTableSession()));
  ...
  useEffect(() => {
    Promise.all([readOrderCatalogAsync(), readSettingsAsync(), readOrderSettingsAsync()]).then(([nextCatalog, nextSettings, nextOrderSettings]) => {
      ...
      setFulfillment((current) => {
        if (readTableSession() && nextOrderSettings.fulfillmentModes.includes('DINE_IN')) return 'DINE_IN';
        return nextOrderSettings.fulfillmentModes.includes(current) ? current : nextOrderSettings.fulfillmentModes[0];
      });
  ...
  useEffect(() => {
    const fromUrl = sanitizeTableId(new URLSearchParams(window.location.search).get('mesa'));
    const nextTableId = fromUrl || readTableSession();
    if (fromUrl) writeTableSession(fromUrl);
    if (!nextTableId) return;
    setFields((current) => ({ ...current, tableId: nextTableId }));
    setTableLocked(true);
  }, []);
