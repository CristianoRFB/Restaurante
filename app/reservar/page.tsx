...
import { createReservationAsync, readSettingsAsync } from '@/lib/baru-repository';
import { photoUrls, settings as fallbackSettings } from '@/lib/baru-data';
import { generateTimeSlots, normalizeWhatsapp, validateReservation, type RestaurantSettings } from '@/shared/baru-domain';

export default function ReservePage() {
  const [settings, setSettings] = useState<RestaurantSettings>(fallbackSettings);
  const [form, setForm] = useState({ date: new Date(Date.now() + 3 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10), time: '', partySize: '2', name: '', whatsapp: '', note: '' });
  const [error, setError] = useState('');
  const [submitting, setSubmitting] = useState(false);

  useEffect(() => {
    let active = true;
    readSettingsAsync().then((next) => { if (active) setSettings(next); }).catch(() => { if (active) setError('Não foi possível carregar os horários atuais. Tente novamente.'); });
    return () => { active = false; };
  }, []);

  const slots = useMemo(() => generateTimeSlots(settings, form.date), [settings, form.date]);
