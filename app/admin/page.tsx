...
import { ArrowRight, CalendarDays, Clock3, Plus, ShoppingBag, Star, UsersRound } from 'lucide-react';
...
import { readOrdersAsync } from '@/lib/order-repository';
import { settings as fallbackSettings } from '@/lib/baru-data';
import { isFirebaseDataMode } from '@/lib/firebase-client';
import { dateKeyInTimeZone, generateTimeSlots, type Reservation, type RestaurantSettings } from '@/shared/baru-domain';
import type { OrderRecord } from '@/shared/order-domain';

function localDate(timezone = 'America/Sao_Paulo') {
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => { let active = true; Promise.all([readReservationsAsync(), readSettingsAsync(), readOrdersAsync()]).then(([nextReservations, nextSettings, nextOrders]) => { if (!active) return; setItems(nextReservations); setSettings(nextSettings); setOrders(nextOrders); }).catch(() => { if (active) setError('Não foi possível sincronizar os dados operacionais.'); }); return () => { active = false; }; }, []);
  const today = useMemo(() => items.filter((reservation) => reservation.date === localDate(settings?.timezone)).sort((left, right) => left.time.localeCompare(right.time)), [items, settings?.timezone]);
